...
class Shopware_Controllers_Frontend_PaymentProcessorCsrf extends Shopware_Controllers_Frontend_Payment implements CSRFWhitelistAware
{
    // Payment status
    ...
    * @return string
    */
    public function getSnippet($name, $localeId, $namespace = 'frontend/payment_processor_csrf/result', $shopId = 1)
    {
    ...
    */
    private function redirectError($errorIdentifier, $redirect = false, $useTranslation = true)
    {
        if ($redirect) {
             $this->redirect(array(
            'controller' => 'payment_processor_csrf',
            'action' => 'result',
    ...
    */
    private function determinePaymentStatus($resultJson)
    {
        ...
        if ($successReview) {
            return Shopware_Controllers_Frontend_PaymentProcessorCsrf::VRPAY_INREVIEW;
        } else {
            if ($resultJson['paymentType'] == "PA") {
                return Shopware_Controllers_Frontend_PaymentProcessorCsrf::VRPAY_PREAUTHORIZATION;
            }
            return Shopware_Controllers_Frontend_PaymentProcessorCsrf::VRPAY_PAYMENTACCEPTED;
        }
    }
    ...
    */
    private function getPaymentFormPath()
    {
        ...
        return "frontend/payment_processor_csrf/form/".$template.".tpl";
    }
}
